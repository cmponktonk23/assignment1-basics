...
class MultiHeadSelfAttention(torch.nn.Module):
    ...
    def forward(self, 
        ...
        weight_combine = torch.cat([self.q_proj.weight, self.k_proj.weight, self.v_proj.weight], dim=0)
        QKV = einsum(in_features, weight_combine, "... sequence_length d_in, d_qkv d_in -> ... sequence_length d_qkv")
        Q, K, V = QKV.split((self.q_proj.weight.size(-1), self.k_proj.weight.size(-1), self.v_proj.weight.size(-1)), dim=-1)
        ...
        return einsum(attention, self.output_proj.weight, "... sequence_length d_v, d_model d_v -> ... sequence_length d_model")
